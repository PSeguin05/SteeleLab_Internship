...
class automatic_tuning_PNA:
    ...
    def __init__(self,
        ...
        """
        self.devices['VNA'] = PNAN5222A(self.parameters['vna_ip'],
                                        reset = True,
    ...
    def _vna_measure(self):
        ...
        self.devices['VNA'].ClearAll()
        self.devices['VNA'].AddTraces(self.map_metadata['trace'])
        self.devices['VNA'].SetContinuous(False)
        freq_start = self.map_metadata['center_frequency'] - self.map_metadata['span'] / 2
        freq_stop = self.map_metadata['center_frequency'] + self.map_metadata['span'] / 2
        self.devices['VNA'].SetRange(freq_start, freq_stop)
        self.devices['VNA'].SetPower(self.map_metadata['power'])
        self.devices['VNA'].SetIFBW(self.map_metadata['ifbw'])
        self.devices['VNA'].SetPoints(self.map_metadata['points'])
        self.devices['VNA'].write('SOUR1:POW' + self.map_metadata['trace'][-1] + ':MODE ON')
        time.sleep(.5)
        ...
        for i, position in enumerate(tqdm(self.zaber_positions)):
            ...
            self.dfol.add_stlab_trace(dataset_name, data, i, autosave_raw=False,autosave_xarray=True )

        self.devices['VNA'].write('SOUR1:POW' + self.map_metadata['trace'][-1] + ':MODE OFF')
    ...
    def cavity_map(self,
        ...
        print(f"VNA span: {self.map_metadata['span'] * 1e-06:.6f} MHz")
    ...
    def _plot_map(self,
        ...
        ax: default = None,
            If not None, provide the axis for the plot
        kwargs:
            Optional parameters for the theory plot
        """
        if ax is None:
            fig, ax = plt.subplots()
        
        self.data_xr.mag.plot(ax=ax, cmap='RdBu_r')
    ...
    def _fit_branches(self,
        ...
        self.analysis.fit_branches(
            bandwidth = self.fit_metadata['span'] * 1e-09, # GHz
    ...
    def _compute_results(self,
        ...
        if plot:
            if ax is None:
                fig, ax = plt.subplots()
            self.analysis.plot_data_results(x='f0', y='kt', ax=ax, **kwargs)
    ...
    def plot_cavity_map(self,
        ...
        self._plot_map(plot_theory = plot_theory, ax=ax, **kwargs)
    ...
    def fit_map(self,
        ...
        self._fit_branches(mode_ids = mode_ids,
                           plot_mask = plot_mask)
        self._compute_results(plot = plot_results, ax=ax, **kwargs)
    ...
    def _vna_measure_opt(self,
        ...
        self.devices['VNA'].ClearAll()
        self.devices['VNA'].AddTraces(self.opt_metadata['trace'])
        self.devices['VNA'].SetContinuous(False)
        freq_start = self.opt_metadata['center_frequency'] - self.opt_metadata['span'] / 2
        freq_stop = self.opt_metadata['center_frequency'] + self.opt_metadata['span'] / 2
        self.devices['VNA'].SetRange(freq_start, freq_stop)
        self.devices['VNA'].SetPower(self.opt_metadata['power'])
        self.devices['VNA'].SetIFBW(self.opt_metadata['ifbw'])
        self.devices['VNA'].SetPoints(self.opt_metadata['points'])
        self.devices['VNA'].write('SOUR1:POW' + self.opt_metadata['trace'][-1] + ':MODE ON')
        time.sleep(.5)
        ...
        self.dfol.add_stlab_trace(dataset_name, data, 0, autosave_raw=False,autosave_xarray=True )
    ...
    def _fit_S21(self):
        ...
        if self.opt_metadata['circle_fit']:
            ...
            try:
                ...
                self.dict_main_params['freq'].append(f0*1e+09)
            ...
            f0 = data_xr_opt.isel(freq=data_xr_opt.mag.argmin('freq')).freq.values[0]
            self.dict_main_params['freq'].append(f0*1e+09)
            self.dict_main_params['kt'].append(np.nan)
